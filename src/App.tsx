...
function Loader({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    const end = 100;
    const duration = 750;     const stepTime = duration / end;
    
    const timer = setInterval(() => {
      setProgress((prev) => {
        if (prev >= end) {
          clearInterval(timer);
          setTimeout(() => setExit(true), 150);           setTimeout(onDone, 650);           return end;
        }
        return prev + 1;
      });
    }, stepTime);
  ...
  return (
    <div className={`loader-screen ${exit ? "loader-exit" : ""}`}>
      <div className="loader-card">
        <div className="loader-progress-num">{progress}%</div>
...
function Nav({ currentRoute }: { currentRoute: string }) {
  ...
  return (
    <nav className={`fixed top-0 inset-x-0 z-50 px-4 py-3 md:px-8 transition-all duration-300 ${scrolled ? "bg-[#faf6ee] shadow-[0_2px_10px_rgba(26,26,26,0.06)] border-b-2 border-dashed border-black/15" : "bg-transparent"}`}>
...
function About() {
  return (
    <section id="about" className="relative py-28 px-6 md:px-10">
      <div className="absolute top-6 left-6 ghost-num text-[16rem] hidden lg:block">01</div>
...
function Ventures() {
  const ventures = [
    { name: "NuroVed", emoji: "🏥", tagline: "Healthcare Infrastructure", status: "v1 Active", color: "sticky-pink", rot: -1.0, desc: "Eliminates fragmentation in patient experience, clinical workflows, and accessibility.", stack: ["FastAPI", "Flutter", "NLP", "AI/ML"] },
    { name: "Educle", emoji: "🎓", tagline: "EdTech + AI Credibility", status: "Building", color: "sticky-blue", rot: 1.2, desc: "End-to-end operations management with AI-driven student credibility systems.", stack: ["React", "Supabase", "Analytics", "AI"] },
    { name: "Trashee", emoji: "🌱", tagline: "CleanTech Community", status: "Building", color: "sticky-green", rot: -0.8, desc: "Community-driven environmental reporting at scale. Turns citizens into change-makers.", stack: ["IoT", "Geo-mapping", "Firebase", "Gamification"] },
    { name: "Actora", emoji: "🧠", tagline: "Behavioral Design AI", status: "Building", color: "sticky-purple", rot: 1.0, desc: "Attacks the dopamine loop directly. Rewires triggers that cause drift.", stack: ["Behavioral AI", "React Native", "Habit Loops"] },
  ];
  return (
    <section id="ventures" className="relative py-28 px-6 md:px-10 crosshatch-bg">
      ...
      <div className="mx-auto max-w-7xl relative z-10">
        <SectionHeader num="03" label="active ventures" note="real products for real problems." />
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12">
          {ventures.map((v, i) => (
            <div
              key={v.name}
              className={`reveal ${i === 0 ? "md:col-span-3" : "md:col-span-1"}`}
            ...
            >
              <div className={`sticky-note ${v.color} !p-6 relative min-h-[260px] card-3d glow-hover`} style={{ transform: `rotate(${v.rot}deg)` }}>
                ...
                <div className={i === 0 ? "md:grid md:grid-cols-12 md:gap-8 items-center" : ""}>
                  <div className={i === 0 ? "md:col-span-7" : ""}>
                  ...
                  <div className={i === 0 ? "md:col-span-5 mt-4 md:mt-0 flex flex-col justify-between h-full" : ""}>
                    {i === 0 && (
                      <div className="hidden md:flex justify-end mb-4">
                        <span className="font-[Patrick_Hand] text-xs px-3 py-1 border-2 border-black rounded-full bg-black text-[#fff9b0]">{v.status}</span>
                      </div>
                    )}
                    <p className="font-[Kalam] text-base leading-snug">{v.desc}</p>
                  </div>
                </div>
...
function Skills() {
  ...
  return (
    <section id="skills" className="relative py-28 px-6 md:px-10">
      ...
      <div className="mx-auto max-w-7xl relative z-10">
        <SectionHeader num="05" label="tech i wield" note="weapons of mass creation." />
...
function Journey() {
  ...
  return (
    <section id="journey" className="relative py-28 px-6 md:px-10 crosshatch-bg">
      ...
      <div className="mx-auto max-w-7xl relative z-10">
        <SectionHeader num="06" label="the journey" note="an open book. unfiltered." />
...
function Currently() {
  ...
  return (
    <section className="relative py-28 px-6 md:px-10 dark-section overflow-hidden">
      <div className="absolute top-6 right-6 ghost-num text-[16rem] !text-white/[0.03] hidden lg:block">09</div>
...
function FunFacts() {
  ...
  return (
    <section id="fun" className="relative py-28 px-6 md:px-10">
      <div className="absolute bottom-6 left-6 ghost-num text-[16rem] hidden lg:block">10</div>
      <div className="mx-auto max-w-7xl relative z-10">
        <SectionHeader num="10" label="beyond the code" note="the human behind the commits." />
...
function Contact() {
  ...
  return (
    <section id="contact" className="relative py-28 px-6 md:px-10">
      <div className="absolute top-6 right-6 ghost-num text-[16rem] hidden lg:block">12</div>
      <div className="mx-auto max-w-5xl relative z-10 text-center">
        <SectionHeader num="12" label="let's build" note="direct from the desk." />
        ...
        <div className="reveal mt-10 flex flex-col sm:flex-row gap-4 justify-center items-center">
          ...
          <a
            ...
            className="btn-hand btn-red text-xl"
          >
            📞 call me directly
          </a>
        </div>
...
function PrivacyModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 bg-[#18130e]/80 animate-[fadeIn_0.2s_ease-out]">
      <div className="relative w-full max-w-lg bg-[#fffdf5] border-[2.5px] border-black rounded-md p-6 md:p-8 shadow-[10px_12px_0_rgba(0,0,0,0.3)] font-[Patrick_Hand] text-lg text-neutral-800">
        <div className="tape absolute -top-4 left-1/2 -translate-x-1/2 w-28 h-6 rotate-[-1deg]" />
        <button 
          onClick={onClose}
          className="absolute top-4 right-4 text-neutral-500 hover:text-black font-bold text-2xl"
          aria-label="Close modal"
        >
          ✕
        </button>
        <h3 className="font-[Permanent_Marker] text-2xl text-[#e63946] mb-4">
          🔒 Privacy &amp; Data Notice
        </h3>
        <div className="space-y-4">
          <p>
            Your privacy is highly respected. This website operates on a <strong>zero-data collection policy</strong>:
          </p>
          <ul className="list-disc list-inside space-y-2">
            <li><strong>No Tracking Cookies:</strong> No persistent cookies are saved or used to track your browsing behavior.</li>
            <li><strong>Form Submissions:</strong> Information sent via the Contact form is processed securely using end-to-end encryption via Web3Forms and is never shared or stored in a public database.</li>
            <li><strong>Local Safety:</strong> All features run fully client-side inside your browser sandbox.</li>
          </ul>
          <p className="text-sm text-neutral-500 font-[Architects_Daughter] mt-4">
            Mohammad Fazil Malek — Ahmedabad, Gujarat
          </p>
        </div>
        <div className="mt-6 text-center">
          <button 
            onClick={onClose}
            className="btn-hand btn-filled text-base px-6 py-2 rounded font-bold"
          >
            I Understand
          </button>
        </div>
    ...
    <footer className="relative px-6 md:px-10 py-10 border-t-[2.5px] border-black/80 bg-[#fffdf5]">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex flex-wrap justify-center gap-4 font-[Patrick_Hand] text-lg">
          ...
          <span>•</span>
          <a href="#/certificates" className="hover:text-[#e63946] transition-colors">Certificates &amp; Achievements</a>
          <span>•</span>
          <a href="#/contact" className="hover:text-[#e63946] transition-colors">Contact &amp; Enquiry Form</a>
          <span>•</span>
          <button onClick={onPrivacyClick} className="hover:text-[#e63946] transition-colors cursor-pointer bg-transparent border-none p-0 font-[Patrick_Hand] text-lg">
            Privacy &amp; Cookies
          </button>
        </div>
...
function NotFoundPage() {
  return (
    <div className="py-24 px-6 md:px-10 max-w-4xl mx-auto text-center flex flex-col items-center justify-center min-h-[65vh] font-[Kalam]">
      <div className="reveal wiggle-box bg-[#fffef7] p-10 md:p-14 border-[2.5px] border-black rounded-lg relative shadow-[6px_8px_0_rgba(26,26,26,0.12)]" style={{ transform: "rotate(-1deg)" }}>
        <div className="tape absolute -top-4 left-1/2 -translate-x-1/2 w-24 h-7 rotate-[1deg]" />
        
        <span className="text-7xl block mb-4">🔍</span>
        
        <h1 className="font-[Permanent_Marker] text-4xl md:text-5xl text-[#e63946] mb-4">
          404 - Page Lost in Margins
        </h1>
        
        <p className="font-[Patrick_Hand] text-2xl text-neutral-600 mb-8 max-w-lg mx-auto leading-relaxed">
          Oops! Looks like this page got sketched out or was never written in our notebook.
        </p>
        
        <a
          href="#/"
          className="btn-hand btn-filled text-xl inline-block"
        >
          ← return to home page
        </a>
        
        <div className="absolute -bottom-5 -right-5 w-10 h-10 hidden md:block">
          <Star className="w-full h-full" color="#ffd93d" />
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const renderPageContent = () => {
    ...
    if (route.startsWith("/stories")) {
      const storySlug = route.replace(/^\/stories\/?/, "").replace(/\/$/, "");
      if (storySlug) {
        return <StoryDetailPage slug={storySlug} />;
      }
      return <StoriesPage />;
    }
